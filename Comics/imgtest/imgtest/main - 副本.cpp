...
void pasteOrgantoFace(IplImage *faceimage,asm_shape shape,CvRect organRect,char *strorganfilename)
{
	...
	int targety1 = faceimage->height * 0.455;//目标y
...
void selectFace(IplImage *image,asm_shape shape,int samplenum,char *strfacefilename)
{
	...
	for(i=1;i<=samplenum;i++)
	{
		...
		r.x = 0;
		r.y = faceimage1->height * 0.455;//目标y
		...
		ThresholdOtsu(cropfaceimage,cropfaceimage,CV_THRESH_BINARY_INV);//二值化人脸
			...
			/*Mat tmp(cropfaceimage,true);
		tmp = tmp/255;
		Mat tmpthin = thinImage(tmp);
		tmpthin = tmpthin*255;
		 IplImage *src=&IplImage(tmpthin);
		 cvNamedWindow("呵呵");
		 imshow("呵呵",tmpthin);*/
		...
		double matchvalue = cvMatchShapes(first_contour,first_contour2,CV_CONTOURS_MATCH_I2);
...
int main()
{
	...
	char* strfilename = "男4.jpg";//要读取测试的图像文件名
	int samplenum = 6;
	...
	IplImage *imagetmp = cvLoadImage(strfacefilename);//读取人脸模型，暂时还没选脸型，先用1吧
	...
	pasteOrgantoFace(faceimage,shape,noseRect,strnamenose);//粘贴鼻子
	

	cvNamedWindow("动漫", 0);
	cvShowImage("动漫", faceimage);	
	...
	cvNamedWindow("左眼处理", 1);
	cvShowImage("左眼处理", lefteyeimageproc);	
	...
	cvNamedWindow("鼻子处理", 1);
	cvShowImage("鼻子处理", noseimageproc);	

	cvNamedWindow("嘴巴", 1);
	cvShowImage("嘴巴", mouthimage);	
	cvNamedWindow("嘴巴处理", 1);
	cvShowImage("嘴巴处理", mouthimageproc);
*/
	cvWaitKey(0);			
